...
def test(args, validLoader, sam_model, epoch=0):
    sam_trans = ResizeLongestSide(args.input_size)
    alldice = []

    test_dice = []
    tqdmtest = tqdm(validLoader)
    sam_model.eval()
    for iteration, (image, mask, bbox, bbox_gt, id) in enumerate(tqdmtest):
        ...
        if iteration == 1:
            save_predicts = torch.zeros_like(mask)
            save_predicts[0] = predicted_masks[:, :, 0:512, 0:512]
            save_predicts[1] = predicted_masks[:, :, 0:512:, 512:]
            save_predicts[2] = predicted_masks[:, :, 512:, 0:512]
            save_predicts[3] = predicted_masks[:, :, 512:, 512:]
            save_image(save_predicts.float(), f'imgs/{9 - args.domain}_predictions_e_{epoch + 1}.png')
        
        dice = dice_score(predicted_masks, merged_masks)
        test_dice.append(dice.detach().item())

        # Update the progress bar
        tqdmtest.set_description(f"Testing on domain{9 - args.domain}")
...
def main():
    ...
    testset = Prostate(base_dir=args.data_path, src_domain=args.domain, split='train', domain_idx=9 - args.domain)
    testLoader = DataLoader(testset, batch_size=4, shuffle=False, num_workers=4, pin_memory=True,drop_last=True)

    sam_model = sam_model_registry[args.vit_name](checkpoint=args.sam_ckpt).cuda()
    # torch.save(sam_model.state_dict(), 'sam.pth')
    # sam_model1 = sam_model_registry[args.vit_name](checkpoint='sam.pth').cuda()
    ...
    val_dice, alldice = test(args, testLoader, sam_model)
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch + 1) % 10 == 0:
            val_dice, alldice = test(args, testLoader, sam_model, epoch)
            if val_dice > maxdice:
                maxdice = val_dice
                maxalldice = alldice
                max_epoch = epoch + 1
                torch.save(sam_model.state_dict(), f'best_{args.domain}_sam.pth')
    ...
    with open(f'4_bbox_results.txt','a') as f:
        f.write(f'trained on domain {args.domain}, train test threshold {args.thresh}\n')
